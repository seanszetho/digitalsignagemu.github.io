requirejs.config({
  paths: {
      'firebase-app': 'https://www.gstatic.com/firebasejs/9.10.0/firebase-app',
      'firebase-database': 'https://www.gstatic.com/firebasejs/9.10.0/firebase-database',
      'jquery': 'path/to/jquery', // Adjust the path as necessary
      'helpers': 'assets/helpers'
  }
});

require(['firebase-app', 'firebase-database', 'jquery', './firebaseConfig.js', 'helpers'], function (firebaseApp, firebaseDatabase, $, firebaseConfig, helpers) {
  ...
  // VARIABLES
  let _config = {};
  let _helpers = helpers.getHelpers();
  let _lastMinute = -1;
  let _lastSecond = -1;
  ...
  document.addEventListener("DOMContentLoaded", function() {
          ...
          form.addEventListener("submit", async function(event) {
              ...
              if (slideId) {
                  try {
                      ...
                      await set(ref(db, 'settings/numSlides'), { numSlides: numSlides });
      ...
      // Fetch slide key and number of slides from Realtime Database and initialize page
      const slideKeyRef = ref(db, 'settings/slideKey');
      const numSlidesRef = ref(db, 'settings/numSlides');
      get(slideKeyRef).then((snapshot) => {
          if (snapshot.exists()) {
              const slideId = snapshot.val().slideKey;
              get(numSlidesRef).then((snapshot) => {
                  if (snapshot.exists()) {
                      const numSlides = snapshot.val().numSlides;
                      const iframe = document.querySelector('.dias-frame');
                      if (iframe) {
                          _config.googleSlide.reloadSlide = (parseInt(numSlides) || 10) * _config.googleSlide.durationSek;
                      ...
                      console.log("No number of slides data available");
                  }
              });
          } else {
