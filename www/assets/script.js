...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (form) {
      form.addEventListener("submit", async function(event) {
          ...
          if (slideId) {
              try {
                  await db.ref('settings/slideKey').set({ slideKey: slideId });
                  await db.ref('settings/numSlides').set({ numSlides: numSlides });
  ...
  const slideKeyRef = db.ref('settings/slideKey');
  const numSlidesRef = db.ref('settings/numSlides');
  slideKeyRef.once('value').then((snapshot) => {
      if (snapshot.exists()) {
          const slideId = snapshot.val().slideKey;
          numSlidesRef.once('value').then((snapshot) => {
              if (snapshot.exists()) {
